...
function extractBookingName(text: string): string {
    ...
    const patterns = [
        ...
        /(?:^|\n|[\*\•\-])\s*(?:_{0,2})(?:Buyer|Purchased by|Ordered by|User Name|Customer Name|Player Name|Booking Name|Name|Customer|Player|Booked by)(?:_{0,2})\s*[\s:]+\s*([A-Za-z]+(?:\s+[A-Za-z]+)*)/i,
...
function extractPaidAmount(text: string): string {
    const patterns = [
        // Hudle/Playo specific labels (Standard & Underscore variants)
        /(?:_{0,2})(?:Amount Paid|Total Amount Paid|Advance Paid)(?:_{0,2})[\s:]+(?:Rs\.?|₹|\u20B9|&#8377;|INR)?\s*(\d+(?:,\d+)*(?:\.\d{2})?)/i,
...
export async function syncEmails(depth: string = 'standard') {
    let connection: any;
    try {
        console.log('[SyncService] Starting Email Check...');

        if (!process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD) {
            console.error('[SyncService] Error: Missing email credentials in environment.');
        ...
        console.log('[SyncService] Connecting to IMAP...');
        connection = await imaps.connect(config);
        await connection.openBox('INBOX');

        const daysToSync = depth === 'all' ? 120 : 30; // Deep Sync 4 months, Standard 1 month

        console.log(`[SyncService] Syncing last ${daysToSync} days...`);
        ...
        console.log(`[SyncService] Found ${messages.length} messages. New/Stale: ${recentMessages.length}`);
        ...
        console.error('[SyncService_Fatal] Sync failed:', error.message);
        // Ensure connection is closed on error to prevent hanging handles
        if (connection) {
            try { connection.end(); } catch (e) { }
        }
        return { success: false, message: `Sync Error: ${error.message || 'IMAP Timeout or Credential issue'}` };
    } finally {
        if (connection) {
            try { connection.end(); } catch (e) { }
        }
